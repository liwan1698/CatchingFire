...
from algo.model.model_config import BertRelationExtract
from algo.models import TripleExtractData
...
# valid_data = load_data('/Users/wanli/create/project/CatchingFire/backend/data/triple/dev_data_small')
predicate2id, id2predicate = {}, {}

with open('/Users/wanli/create/project/CatchingFire/backend/data/triple/all_50_schemas') as f:
    for l in f:
        l = json.loads(l)
...
# 建立分词器
tokenizer = Tokenizer(BertRelationExtract.VOCAB_PATH, do_lower_case=True)
...
class BertTripleExtract(CustomModel):
    def __init__(self):
        super().__init__()
    ...
    def load_data(self):
        # 从数据库获得数据
        data = TripleExtractData.objects.values("text", "human_tag").exclude(human_tag=None)
        def handle_data(data):
            ...
            D = []
            for l in data:
                spo_list = json.loads(l['human_tag'])
                D.append({
                    'text': l['text'],
                    'spo_list': [(spo['subject'], spo['predicate'], spo['object'])
                                 for spo in spo_list]
            ...
            return D
        split_num = int(len(data) * self.split_rate)
        data = handle_data(data)
        self.train_data = data[:split_num]
        self.test_data = data[split_num:]

    def build(self):
        class Evaluator(keras.callbacks.Callback):
            """评估与保存
            """
            def __init__(self, subject_model, object_model, train_model):
                self.best_val_f1 = 0.
            ...
            def extract_spoes(self, text):
                ...
                if subjects:
                    ...
                    return []
        self.train_generator = data_generator(self.train_data, batch_size)
        # 补充输入
        ...
        bert = build_transformer_model(
            config_path=BertRelationExtract.CONFIG,
            checkpoint_path=BertRelationExtract.CHECK_POINT,
        ...
        self.evaluator = Evaluator(self.subject_model, self.object_model, self.train_model)
    ...
    def predict_all(self):
        output = self.train_model.predict(self.train_generator.forfit())
        # 将结果保存到数据库
        print(output)
