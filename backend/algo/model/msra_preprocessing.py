...
def msra_preprocessing(split_rate: float = 0.8,
    ...
    :return:None
    """
    path_train = os.path.join(BertBilstmCrfConfig.MSRA_DIR, "train.txt")
    path_test = os.path.join(BertBilstmCrfConfig.MSRA_DIR, "test.txt")
    if not ignore_exist and os.path.exists(path_train) and os.path.exists(path_test):
        print("MSRA数据预处理已经完成")
        return
    else:
        print("正在对MSRA数据进行预处理......")
    path_train1 = os.path.join(BertBilstmCrfConfig.MSRA_DIR, "train1.txt")
